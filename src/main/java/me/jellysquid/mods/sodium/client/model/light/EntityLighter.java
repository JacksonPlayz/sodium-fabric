        ...
        for (int bX = bMinX; bX < bMaxX; bX++) {
            ...
            for (int bY = bMinY; bY < bMaxY; bY++) {
                ...
                for (int bZ = bMinZ; bZ < bMaxZ; bZ++) {
                    ...
                    if (blockState.isOpaqueCube(entity.world, pos) && blockState.getLightValue(entity.world, pos) <= 0) {
                        continue;
                    }
                    ...
                    sl += weight * (entity.world.getLightFor(LightType.SKY, pos) / MAX_LIGHT_VAL);

                    if (calcBlockLight) {
                        bl += weight * (entity.world.getLightFor(LightType.BLOCK, pos) / MAX_LIGHT_VAL);
                    } else {
                        bl += weight;
                    }
                }
